...
- **THEN** the row is persisted with a generated UUID, timestamps, and all nullable fields default to NULL
...
- `POST /api/task-profiles` — create a new profile (body: name, description, match_rules, model, system_prompt, max_turns, reasoning_effort, mcp_servers, litellm_mcp_servers, skill_ids)
...
All endpoints SHALL require admin role. The create and update endpoints SHALL validate that `name` is non-empty and unique. The create and update endpoints SHALL validate that `reasoning_effort`, if provided, is one of `low`, `medium`, `high`.
