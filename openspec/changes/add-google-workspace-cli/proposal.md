...
- **Install `gws` CLI in the task-runner Docker image** via npm (`@googleworkspace/cli`) and bundle the upstream `gws-*` agent skills from the `googleworkspace/cli` repo at the matching version tag
