...
**Ask the provider for usage, or measure nothing.** Discovered on the first real run rather than reasoned about in advance: every turn reported zero tokens. The agent SDK sets `stream_options.include_usage` only when it recognises the client as pointing at OpenAI's own endpoint, and errand's points at LiteLLM, so the streaming usage chunk was never requested and `ModelResponse.usage` stayed all zeros. Setting `include_usage=True` on `ModelSettings` fixes it. Without this the whole change reports zeros against errand's normal configuration, so it is load-bearing rather than incidental.

**A usage block of zeros is not a measurement.** The spec anticipated *absent* usage; a provider that reports zeros is a third case, and the badge would have shown a confident `0`. A zero prompt is impossible for a real turn, so it is treated as no report and the token fields are omitted. This is a guard rather than a fix: the fix is asking for usage in the first place, and this stops the failure being invisible if some other provider declines anyway.
...
What did have to be built is the lever: `MAX_CONTEXT_TOKENS` was read from the runner's own environment with a hard-coded 150,000 default and was never set by the server, so there was no way to configure it at all. A `max_context_tokens` setting now resolves through the same path as `compaction_timeout` and `compaction_max_tokens` and is forwarded to the runner. Choosing the right value is still deferred; being *able* to choose it is not, because the pressure thresholds are percentages of it.
