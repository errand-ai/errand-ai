...
- **Why did the message list shrink between compactions?** Observed in the verification run (task `2ece82fc`): compaction 1 saw 184 messages, compaction 2 saw 121 — fewer messages but *more* tokens. Neither explanation on the table accounts for it. If history were rebuilt from the originals the count should grow; if the compacted list persisted, `_is_compaction_summary` would have matched the marker and logged a merge, and it logged a full summarisation. No trim line appears either. The held-summary guard handled it correctly (fell back, then chained from compaction 3 onward), so nothing is broken — but the Context section's account of what the SDK hands back each turn is incomplete, and the answer may change how much of the prefix is reusable.
