...
### Build note — task-runner distroless debian12 → debian13 (discovered in Pass 4)
Moving the task-runner's final stage to `python3-debian13` (trixie) surfaced two runtime-only breakages (build passed; only `docker run` caught them), both from the git-builder's shared-library staging that copies bookworm libs into the distroless base:
1. **`/lib` is a merged-/usr symlink on trixie.** `COPY /staged-libs/ /` failed (`cannot copy to non-directory`) because staged libs had a real top-level `/lib` dir colliding with the base's `/lib`→`/usr/lib` symlink. Fix: rewrite a leading `/lib/` **destination** to `/usr/lib/` while keeping the soname filename. (A first attempt using `readlink -f` for the whole path fixed the collision but staged libs under their *versioned* name instead of the soname, so `git`/`ssh`/`jq` then couldn't find `libpcre2-8.so.0`/`libjq.so.1` — reverted to soname-preserving.)
2. **OpenSSL shadowing.** Staging bookworm's `libcrypto`/`libssl` (OpenSSL 3.0) shadowed the base's, and trixie's Python 3.13 `_ssl` needs `OPENSSL_3.3.0` → `ImportError`. Fix: add `libssl`/`libcrypto` to the skip list so the base's OpenSSL 3.5 is used — it provides both the 3.3.0 symbols Python needs and the older 3.0.0 symbols the staged bookworm git/curl require. (Worked on debian12 only by luck — same OpenSSL 3.0.) Future distroless base bumps should re-run the image smoke test (`python3 -c "import ssl"` + `git/ssh/jq --version`), not just the build.
