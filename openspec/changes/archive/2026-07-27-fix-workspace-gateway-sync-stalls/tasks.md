...
- [x] 8.2 Local stack check via `docker compose -f testing/docker-compose.yml up --build`
      — **superseded by the cluster verification (8.3–8.5, 5.4).** The full `--profile workspace` stack needs an authorised `rclone.conf` and a workspace bearer, and everything it would have exercised was instead verified against the real deployment, which is strictly stronger: the desync repair recovered an actual file, the pinned-handle detection caught an actual stall, and token rotation was confirmed working for the first time since the feature shipped. What *was* checked locally, directly against the built image: it runs as uid 65532, `/cache` and `/config-rw` are writable, rclone creates its cache `0700 gateway:gateway` (the finding behind D1), the reconcile repairs and quarantines correctly against real rclone 1.68, and the entrypoint token check behaves correctly under busybox across five cases.
