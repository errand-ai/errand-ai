...
- **WHEN** a cloud-relayed message has `integration: "jira"` and `endpoint_type: "webhook"`
- **THEN** the dispatcher SHALL call the Jira webhook processing function with the raw body bytes, headers, and the `trigger_id` from the relay message
- **THEN** the dispatcher SHALL re-verify the HMAC signature using the matched trigger's webhook secret
