...
**Choice:** The server exposes a `GET /api/integrations/status` endpoint that reports which cloud storage integrations are available (based on which URL env vars are set and which client credentials are configured). The frontend uses this to enable/grey-out integration cards.
