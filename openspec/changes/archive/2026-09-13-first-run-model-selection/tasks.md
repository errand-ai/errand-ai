...
- **Both Slack intakes tagged `Needs Info` without ever routing on it — now fixed.** `handlers.py` (slash command) and `routes.py` (mention) each constructed their task with `status="pending"` unconditionally, so the tag was displayed and never acted on: a task the classifier said it could not understand was queued and run anyway while the board showed it as needing attention. That predated this change and affected the short-input rule as much as the unusable-classification case added here. Raised in review, initially deferred as a behaviour change, then fixed here after all: `task-categorisation` states the requirement of the capability — "if the task has a 'Needs Info' tag, the task SHALL be set to status `review`" — not of the web endpoint, so leaving two intakes contradicting an archived requirement was the worse option, and writing the requested regression tests against the old behaviour would have enshrined it. Two existing tests built their fixtures from short Slack inputs and had to be rebuilt around a classified one; that they broke is the change being real. An earlier reply on the review thread said `handlers.py` routed correctly — it did not, and that has been corrected on the thread. The fix then had to be fixed once more: position is numbered per column and was being taken before the status was known, which was harmless only while every Slack task was `pending`. Both paths now use the shared `_next_position` helper the web path has always used.
- **Two review findings fixed after archiving, both in the same blind spot — a gate reading state that had already moved.** First: reconciliation clears the model settings of a departed detected provider, and the establisher's "has anybody chosen anything" gate then read the state this same scan had just emptied, so a sole model found elsewhere replaced the operator's choice inside one call. The gate is now read before reconciliation touches anything. Second: a role was treated as configured as soon as its provider id existed, without checking the provider still lists the model — so a dropped model, or one persisted through the generic settings path, reported as configured while every task failed. The model is now checked against the listing, and an unreadable listing invalidates nothing, because unavailable is not absent. The two new scenarios are in `openspec/specs/llm-providers/spec.md`.
