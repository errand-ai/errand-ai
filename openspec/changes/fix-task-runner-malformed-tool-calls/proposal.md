...
- **Sanitize malformed tool calls in `filter_model_input`**: Before each LLM call, scan assistant messages for tool calls with invalid JSON arguments. Attempt to repair truncated JSON (e.g. close unclosed braces/brackets/strings). If repair fails, replace the tool call's arguments with a valid JSON error placeholder (`{"error": "malformed_arguments", ...}`), so LiteLLM can serialize the conversation history and the agent can observe the failure and retry the tool call with valid arguments.
- **Classify API errors as retryable vs non-retryable**: Replace the catch-all `except Exception: sys.exit(1)` with specific error handling that distinguishes transient errors (rate limits, timeouts, connection errors) from non-retryable errors (malformed conversation history, invalid requests). For transient errors, retry with exponential backoff within the task-runner process before giving up. Non-retryable and unknown errors exit immediately with a structured error event.
