...
After Wave 2 no card injects `'settings-state'`. The provide block, the `loadSettings()` orchestration, and the parent-tracked refs in `SettingsPage.vue` can all be deleted. What remains is just the `<SettingsShell>` wrapper.
