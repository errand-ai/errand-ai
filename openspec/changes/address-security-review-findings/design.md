...
**CORS defaults to same-origin, configurable.** errand serves its own frontend from the same origin in production, so a same-origin default breaks nothing in the standard deployment. Development and split-origin deployments configure the origins they need. Keeping `allow_credentials` unset preserves the property that makes the current wildcard harmless.
...
- Is `read_rss_feed` worth the same treatment as `read_url`, or should it share one validated fetch helper? Sharing is cleaner but couples two tools with different failure tolerances.
- Does `errand-cloud` call the API cross-origin? That determines whether the CORS default can be same-origin or must include the cloud origin.
