...
| `raw` | `line` (string) | A line that could not be interpreted as a structured event, preserved verbatim |
| `claude_fallback` | `reason` (string), `terminal_reason` (string or null), `fell_back` (boolean) | Delegation to the Claude Code CLI failed; `fell_back` records whether the standard agent loop then ran |
...
The `agent_start` event's `agent` field SHALL identify the executing agent: `TaskRunner` for the standard agent loop and `claude` for a run delegated to the Claude Code CLI. Consumers that treat a transcript without `agent_start` as an infrastructure failure — notably the eval driver — depend on delegated runs emitting it too.
...
#### Scenario: Agent start event emitted for a delegated run

- **WHEN** the task runner delegates to the Claude Code CLI and receives its init event
- **THEN** stderr contains a line `{"type": "agent_start", "data": {"agent": "claude"}}`
...
#### Scenario: Raw event emitted for an uninterpretable line

- **WHEN** the task runner reads a line from a delegated CLI that is not a recognised event
- **THEN** stderr contains a line `{"type": "raw", "data": {"line": "<original line>"}}`

#### Scenario: Claude fallback event emitted

- **WHEN** delegation to the Claude Code CLI fails before any tool call and the standard agent loop takes over
- **THEN** stderr contains a line `{"type": "claude_fallback", "data": {"reason": "<reason>", "terminal_reason": "api_error", "fell_back": true}}`
...
- **`claude_fallback`**: Displayed as an amber status line naming the reason and stating whether the standard agent loop took over.

Any event type not listed above SHALL be rendered as a plain monospace line rather than discarded, so protocol additions degrade visibly instead of disappearing.
...
#### Scenario: Claude fallback rendered as status line

- **WHEN** the log viewer receives a `claude_fallback` event with reason "Not logged in" and `fell_back: true`
- **THEN** the viewer displays an amber status line naming the reason and indicating the standard agent loop took over
...
#### Scenario: Unknown event type still rendered

- **WHEN** the log viewer receives an event whose type it does not recognise
- **THEN** the viewer displays it as a plain monospace line rather than discarding it
