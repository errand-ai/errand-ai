...
- [ ] 8.5 Capture and hand over, at minimum: a scan on an installation with **no model configured**; a scan where a **sole model was established automatically**, including whatever field names that model — stating it is a requirement on their side and they cannot invent the field; the **provider-list read carrying the configured state**, which is what the card reads on mount; and the model-choice call **accepted**, **refused for a model the provider does not list**, and **refused for a provider that does not exist**
