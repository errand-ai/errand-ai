...
`cloud_auth_login` (`errand/main.py:1764`) generates a CSRF nonce, stores it as the `cloud_auth_state` setting, and returns `{"redirect_url": "{cloud_url}/auth/tenant/login?redirect_uri={our callback}&state={nonce}"}`. `CloudServicePage.vue:109` opens that URL in a 500×600 popup. The user authenticates against the cloud's Keycloak, the cloud redirects back to `GET /api/cloud/auth/callback?code=…&state=…`, which validates the nonce, calls `exchange_code(cloud_url, code)` → `POST {cloud}/auth/tenant/token`, decodes `sub` and `email` from the access token, stores an encrypted `PlatformCredential`, starts the WebSocket client, registers cloud endpoints, and closes the popup with `window.close()`.
