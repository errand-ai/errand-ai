...
The `max(1, ...)` clamp on `task_log_buffer_max_entries` and `task_log_buffer_ttl_seconds` SHALL remain in the task manager (a Valkey `LTRIM`/`EXPIRE` invariant), applied after resolution. The same `max(1, ...)` clamp SHALL also apply to `max_concurrent_tasks` to prevent a deadlocked `Semaphore(0)` if the DB or env ships an out-of-range value.
...
#### Scenario: max_concurrent_tasks clamped after resolution

- **WHEN** the resolver returns `0` for `max_concurrent_tasks` (from a stale DB row or env override)
- **THEN** the task manager clamps the value to `1` and logs a WARNING; the semaphore is never sized below `1`
