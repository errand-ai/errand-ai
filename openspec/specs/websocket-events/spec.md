...
WebSocket endpoint that pushes real-time task-board events to connected frontend clients, backed by Valkey pub/sub for cross-replica broadcasting. (Per-task log streaming was migrated to the SSE endpoint — see `sse-task-events`.)
