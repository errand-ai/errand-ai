...
Each task SHALL have an `execute_at` field (timestamptz, nullable) storing when the task should next be executed. For `immediate` tasks, `execute_at` SHALL be set to the current time at creation. For `scheduled` and `repeating` tasks, `execute_at` SHALL be set to the time extracted by the LLM from the task description. If no time can be extracted, `execute_at` SHALL be null.

#### Scenario: Immediate task gets current time
- **WHEN** a task is created with category `immediate`
- **THEN** `execute_at` is set to approximately `now()`
