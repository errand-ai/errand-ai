...
MCP tools (`slack_message`, `slack_reply`) that let the task-runner originate Slack messages — posting to channels, DMing users, or replying in threads. Server-mediated: the workspace bot token never leaves the server.
