...
4. **final** (`gcr.io/distroless/python3-debian12:nonroot`): Copy Python packages from the python builder, staged binaries and libraries from git-builder (git, ssh, curl, busybox, gh), and node binary plus openspec from node-builder. The working directory SHALL be `/workspace`. The entrypoint SHALL be `["python3", "/app/main.py"]`.

The `gh` version and `openspec` version SHALL be configurable via Docker build args with sensible defaults.

The following binaries SHALL be available in the final image: `git`, `ssh`, `ssh-keygen`, `ssh-keyscan`, `curl`, `gh`, `node`, `openspec`, and busybox applets (`sh`, `cat`, `ls`, `grep`, etc.).
