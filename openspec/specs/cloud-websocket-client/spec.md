...
The cloud WebSocket client maintains the errand-server's persistent connection to Errand Cloud, handling registration, HTTP proxying, real-time event subscriptions, OAuth token relay, and payment/subscription alerts over a single tunnel.
## Requirements
