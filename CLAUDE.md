...
- **Agent defaults (`max_turns`, `reasoning_effort`)**: registry settings (defaults `200` / `medium`) resolved env → DB → default via `resolve_setting_value` in `_read_settings` — not the select list — and forwarded as `MAX_TURNS` / `REASONING_EFFORT` unless the profile overrides them. `GET /api/worker/defaults` uses the same resolver, so the profile modal's "Deployment default" is the value applied. The env vars still win and lock the field in the UI, which is why neither compose file nor the Helm default sets `MAX_TURNS` any more. A `null` PUT deletes the stored override. Edited on the Task Management tab (`TaskManagementCard`, ui-components ≥0.23.0) alongside `timezone` and `task_runner_log_level`; `PUT /api/settings` validates all four (`_SETTING_VALIDATORS` in `main.py`).
